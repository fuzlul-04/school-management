...
<div class="space-y-6">
    ...
    <form action="{{ route('roles.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 space-y-4">
        @csrf
        ...
        <div>
            ...
            <input type="number" name="priority" value="{{ old('priority', 1) }}" min="1" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            @error('priority')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
