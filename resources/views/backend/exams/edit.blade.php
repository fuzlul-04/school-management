...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Exam</h1>
        ...
        <form method="POST" action="{{ route('exams.update', $exam) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $exam->name }}" class="w-full border rounded px-3 py-2" required>
                </div>
                ...
                <div>
                    ...
                    <select name="type" class="w-full border rounded px-3 py-2" required>
                        ...
                        <option value="{{ $value }}" {{ $exam->type == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ $exam->start_date->format('Y-m-d') }}" class="w-full border rounded px-3 py-2" required>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">End Date</label>
                    <input type="date" name="end_date" value="{{ $exam->end_date->format('Y-m-d') }}" class="w-full border rounded px-3 py-2" required>
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Remarks</label>
                <textarea name="remarks" rows="3" class="w-full border rounded px-3 py-2">{{ $exam->remarks }}</textarea>
            </div>
